...
uses
  ...
  RpDefine, RpRave, RpCon, RpConDS, RpBase, RpSystem;
...
type
  TMastData = class(TDataModule)
    ...
    MastRvProject: TRvProject;
    CustByLastInvRvDataSetConnection: TRvDataSetConnection;
    MastRvSystem: TRvSystem;
    OrdersByDateRvDataSetConnection: TRvDataSetConnection;
  ...
  public
    ...
    procedure PrintOrderReport(const Preview: Boolean; const FromDate: TDateTime; const ToDate: TDateTime);
  end;
...
begin
//TODO: DXE2 Set right path to DB
// Assume data is in C:\Program Files\Common Files\CodeGear Shared\Data\
  Result := ExpandFileName('C:\Program Files (x86)\Common Files\CodeGear Shared\Data\');
end;
...
procedure TMastData.PrintCustomerReport(Preview: Boolean);
begin
  CustByLastInvQuery.Open;
  MastRvProject.SelectReport('CustomerByInvoiceReport', True);
  if Preview then
    MastRvSystem.DefaultDest := rdPreview
  else
    MastRvSystem.DefaultDest := rdPrinter;
  MastRvProject.Execute;
  CustByLastInvQuery.Close;
end;

procedure TMastData.PrintOrderReport(const Preview: Boolean; const FromDate: TDateTime; const ToDate: TDateTime);
const
  FromToHeading = 'From ''%s'' To ''%s''';
begin
  OrdersByDateQuery.Close;
  OrdersByDateQuery.Params.ParamByName('FromDate').AsDateTime := FromDate;
  OrdersByDateQuery.Params.ParamByName('ToDate').AsDateTime := ToDate;
  OrdersByDateQuery.Open;

  // Format the From To header with the user's dates entered
  MastRvProject.SetParam('OrdersFromToHeading', Format(FromToHeading, [DateToStr(FromDate), DateToStr(ToDate)]));
  MastRvProject.SelectReport('OrdersByDateReport', True);
  if Preview then
    MastRvSystem.DefaultDest := rdPreview
  else
    MastRvSystem.DefaultDest := rdPrinter;
